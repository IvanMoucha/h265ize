...
class Video {
    ...
    filesystem() {
        ...
        return new Promise(function (resolve, reject) {
            ...
            if (fse.existsSync(_self.output.path)) {
    ...
    encode() {
        ...
        return new Promise(function (resolve, reject) {
            ...
            if (!_self.options.delete)
                fse.ensureDir(_self.output.dir, function (err) {
                    if (err) {
            ...
            _self.ffmpegCommand
                .on('progress', function (progress) {
                    ...
                    let estimatedFileSize = precent > 10 ? filesize(fse.statSync(_self.output.path).size / precent * 100) : '';
    ...
    multiPass() {
        ...
        return new Promise(function (resolve, reject) {
            ...
            return fse.move(_self.output.path, newInput, {
                clobber: true
            }, function (err) {
                if (err) {
    ...
    verifyEncode() {
        ...
        return new Promise(function (resolve, reject) {
            Video.getMetadata(_self, true).then(function (metadata) {
                ...
                const timeDiffLimit = 10; // in seconds
                ...
                //     fse.unlinkSync(_self.output.path);
    ...
    move() {
        ...
        return new Promise(function (resolve, reject) {
            ...
            fse.remove(_self.path, function (err) {
                if (err) return reject(err);
                _self.encoder.watchIgnore.push(newDestination);
                fse.move(_self.output.path, newDestination,
                    function (err) {
                        if (err) return reject(err);
        ...
        return new Promise(function (resolve, reject) {
            _.each(_self.temp.files, function (path, i) {
                fse.pathExists(path)
                    .then(ret => {
                        if (ret) {
                            fse.unlink(path).catch(err => console.error(err))
                        }
                    });
            });
            resolve();
    ...
    static takeScreenshots(path, destination, logger) {
        return new Promise(function (resolve, reject) {
            ...
            fse.ensureDir(outputDir, function (err) {
                if (err) {
                    throw err;
                }
